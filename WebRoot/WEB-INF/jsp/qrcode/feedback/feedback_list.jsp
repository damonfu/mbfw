...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="fbman/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>fbman/delete.do?id="+Id+"&tm="+new Date().getTime();
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>fbman/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>fbman/excel.do';
		}
		</script>
		<style type="text/css">
